 ...
 *          Based on: https://github.com/eziya/STM32_HAL_I2C_HD44780/blob/master/Src/liquidcrystal_i2c.h
 *
 * @date    2025-09-28
...
#define LCD_CMD_ENTRYMODESET 0x04   /**< Command to set entry mode (text direction, shifting) */
#define LCD_CMD_DISPLAYCONTROL 0x08 /**< Command to control display on/off, cursor, blink */
#define LCD_CMD_CURSORSHIFT 0x10    /**< Command to shift cursor or entire display */
#define LCD_CMD_FUNCTIONSET 0x20    /**< Command to set function (data length, lines, font) */
#define LCD_CMD_SETCGRAMADDR 0x40   /**< Command to set CGRAM address (for custom characters) */
#define LCD_CMD_SETDDRAMADDR 0x80   /**< Command to set DDRAM address (cursor position) */

/* Entry mode flags (for LCD_CMD_ENTRYMODESET) */
#define LCD_ENTRY_RIGHT 0x00           /**< Entry mode: increment DDRAM addr right-to-left */
#define LCD_ENTRY_LEFT 0x02            /**< Entry mode: increment DDRAM addr left-to-right */
#define LCD_ENTRY_SHIFT_INCREMENT 0x01 /**< Entry mode: shift display when writing new data */
#define LCD_ENTRY_SHIFT_DECREMENT 0x00 /**< Entry mode: do not shift display on write */
...
#define LCD_PIN_RW 0x00     /**< Expander bit for Read/Write (RW) pin (unused in write-only mode) */
 ...
 * @param  charmap  Pointer to 8 bytes; each byte uses bits 0..4 for a 5-pixel row
