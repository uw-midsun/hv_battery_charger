 ...
 * The instances for Real FFT and FFT, cosine values table and twiddle factor
 *table are stored in an instance data structure. A separate instance structure
 *must be defined for each transform. There are separate instance structure
 ...
 *arm_cfft_radix4_f32() and arm_rfft_f32() respectively for details regarding
