...
typedef struct {
  ...
  uint32_t BankNumber; /*!< Select the start slave bank filter
                            F3 devices don't support CAN2 interface (Slave).
                          Therefore this parameter is meaningless but it has
                          been kept for compatibility across STM32 families */
...
typedef struct {
  ...
  uint32_t IDE; /*!< Specifies the type of identifier for the message that will
                   be received. This parameter can be a value of @ref
                   CAN_identifier_type */
  ...
  uint32_t FMI; /*!< Specifies the index of the filter the message stored in the
                   mailbox passes through. This parameter must be a number
...
typedef struct {
  ...
  CanRxMsgTypeDef*
      pRxMsg; /*!< Pointer to reception structure for RX FIFO0 msg */

  CanRxMsgTypeDef*
      pRx1Msg; /*!< Pointer to reception structure for RX FIFO1 msg */
...
#define CAN_FILTER_FIFO0 \
  ((uint8_t)0x00U) /*!< Filter FIFO 0 assignment for filter x */
#define CAN_FILTER_FIFO1 \
  ((uint8_t)0x01U) /*!< Filter FIFO 1 assignment for filter x */
/**
 * @}
 */
...
/* @note When SLAK interrupt is disabled (SLKIE=0U), no polling on SLAKI is
...
#define __HAL_CAN_MSG_PENDING(__HANDLE__, __FIFONUMBER__)  \
  (((__FIFONUMBER__) == CAN_FIFO0)                         \
       ? ((uint8_t)((__HANDLE__)->Instance->RF0R & 0x03U)) \
...
#define CAN_TXSTATUS_NOMAILBOX \
  ((uint8_t)0x04U) /*!< CAN cell did not provide CAN_TxStatus_NoMailBox */
