...
typedef struct {
  uint32_t
      FunctionalMode; /*!< Set OPAMP functional mode by setting internal
                         connections: OPAMP operation in standalone, follower,
                         ... This parameter can be a value of @ref
                         OPAMP_LL_EC_FUNCTIONAL_MODE
                           @note If OPAMP is configured in mode PGA, the gain
  ...
  uint32_t
      InputInverting; /*!< Set OPAMP inverting input connection.
                         ...
                         GPIO pin), this parameter is discarded.
...
#define LL_OPAMP_MODE_STANDALONE \
  (0x00000000U) /*!< OPAMP functional mode, OPAMP operation in standalone */
...
#define LL_OPAMP_MODE_PGA_EXT_FILT_IO0                                        \
  (OPAMP_CSR_PGGAIN_3 |                                                       \
   OPAMP_CSR_VMSEL_1) /*!< OPAMP functional mode, OPAMP operation in PGA with \
                         external filtering on OPAMP input IO0. */
#define LL_OPAMP_MODE_PGA_EXT_FILT_IO1                                        \
  (OPAMP_CSR_PGGAIN_3 | OPAMP_CSR_PGGAIN_2 |                                  \
   OPAMP_CSR_VMSEL_1) /*!< OPAMP functional mode, OPAMP operation in PGA with \
                         external filtering on OPAMP input IO1. */
/**
 * @}
 */
...
#define LL_OPAMP_INPUT_NONINVERT_IO0                                          \
  (OPAMP_CSR_VPSEL) /*!< OPAMP non inverting input connected to GPIO pin (pin \
                       PA1 for OPAMP1, pin PA7  for OPAMP2, pin PB0  for      \
                       OPAMP3, pin PB13 for OPAMP4) */
...
#define LL_OPAMP_INPUT_NONINV_DAC1_CH1                                       \
  (LL_OPAMP_INPUT_NONINVERT_IO3) /*!< OPAMP non inverting input connected to \
                                    DAC1 channel1 output (specific to OPAMP  \
                                    instances: OPAMP4) */
#define LL_OPAMP_INPUT_NONINV_DAC1_CH2                                       \
  (LL_OPAMP_INPUT_NONINVERT_IO3) /*!< OPAMP non inverting input connected to \
                                    DAC1 channel2 output (specific to OPAMP  \
                                    instances: OPAMP1, OPAMP3) */
/**
 * @}
 */
...
#define LL_OPAMP_INPUT_INVERT_IO0                                              \
  (0x00000000U) /*!< OPAMP inverting input connected to GPIO pin (pin PC5 for  \
                   OPAMP1, pin PC5 for OPAMP2, pin PB10  for OPAMP3, pin PB10  \
                   for OPAMP4). Note: OPAMP inverting input is used with OPAMP \
                   in mode standalone or PGA with external capacitors for      \
                   filtering circuit. Otherwise (OPAMP in mode follower),      \
                   OPAMP inverting input is not used (not connected to GPIO    \
                   pin). */
#define LL_OPAMP_INPUT_INVERT_IO1                                            \
  (OPAMP_CSR_VMSEL_0) /*!< OPAMP inverting input connected to GPIO pin (pin  \
                         PA3 for OPAMP1, pin PA5 for OPAMP2, pin PB2   for   \
                         OPAMP3, pin PD8  for OPAMP4). Note: OPAMP inverting \
                         input is used with OPAMP in mode standalone or PGA  \
                         with external capacitors for filtering circuit.     \
                         Otherwise (OPAMP in mode follower), OPAMP inverting \
...
#define LL_OPAMP_INPUT_INVERT_CONNECT_NO                                     \
  (OPAMP_CSR_VMSEL_1) /*!< OPAMP inverting input not externally connected    \
                         (intended for OPAMP in mode follower or PGA without \
                         external capacitors for filtering). Note: On this   \
                         STM32 series, this literal include cases of value   \
                         0x11 for mode follower and value 0x10 for mode PGA. \
                       */
/**
 * @}
 */
...
#define LL_OPAMP_INPUT_NONINVERT_IO0_SEC                                       \
  (LL_OPAMP_INPUT_NONINVERT_IO0                                                \
   << (OPAMP_CSR_VPSSEL_Pos -                                                  \
       OPAMP_CSR_VPSEL_Pos)) /*!< OPAMP non inverting input secondary          \
                                connected to GPIO pin (pin PA1 for OPAMP1, pin \
                                PA7  for OPAMP2, pin PB0  for OPAMP3, pin PB13 \
                                for OPAMP4) */
#define LL_OPAMP_INPUT_NONINVERT_IO1_SEC                                       \
  (LL_OPAMP_INPUT_NONINVERT_IO1                                                \
   << (OPAMP_CSR_VPSSEL_Pos -                                                  \
       OPAMP_CSR_VPSEL_Pos)) /*!< OPAMP non inverting input secondary          \
                                connected to GPIO pin (pin PA7 for OPAMP1, pin \
                                PD14 for OPAMP2, pin PB13 for OPAMP3, pin PD11 \
                                for OPAMP4) */
#define LL_OPAMP_INPUT_NONINVERT_IO2_SEC                                       \
  (LL_OPAMP_INPUT_NONINVERT_IO2                                                \
   << (OPAMP_CSR_VPSSEL_Pos -                                                  \
       OPAMP_CSR_VPSEL_Pos)) /*!< OPAMP non inverting input secondary          \
                                connected to GPIO pin (pin PA3 for OPAMP1, pin \
                                PB0  for OPAMP2, pin PA1  for OPAMP3, pin PB11 \
                                for OPAMP4) */
#define LL_OPAMP_INPUT_NONINVERT_IO3_SEC                                       \
  (LL_OPAMP_INPUT_NONINVERT_IO3                                                \
   << (OPAMP_CSR_VPSSEL_Pos -                                                  \
       OPAMP_CSR_VPSEL_Pos)) /*!< OPAMP non inverting input secondary          \
                                connected to GPIO pin (pin PA5 for OPAMP1, pin \
                                PD14 for OPAMP2, pin PA5  for OPAMP3, pin PA4  \
                                for OPAMP4) */
#define LL_OPAMP_INPUT_NONINV_DAC1_CH1_SEC                                     \
  (LL_OPAMP_INPUT_NONINV_DAC1_CH1                                              \
   << (OPAMP_CSR_VPSSEL_Pos -                                                  \
       OPAMP_CSR_VPSEL_Pos)) /*!< OPAMP non inverting input secondary          \
                                connected to DAC1 channel1 output (specific to \
                                OPAMP instances: OPAMP4) */
#define LL_OPAMP_INPUT_NONINV_DAC1_CH2_SEC                                     \
  (LL_OPAMP_INPUT_NONINV_DAC1_CH2                                              \
   << (OPAMP_CSR_VPSSEL_Pos -                                                  \
       OPAMP_CSR_VPSEL_Pos)) /*!< OPAMP non inverting input secondary          \
                                connected to DAC1 channel2 output (specific to \
                                OPAMP instances: OPAMP1, OPAMP3) */
...
#define LL_OPAMP_INPUT_INVERT_IO0_SEC                                          \
  (LL_OPAMP_INPUT_INVERT_IO0                                                   \
   << (OPAMP_CSR_VMSSEL_Pos -                                                  \
       OPAMP_CSR_VMSEL_Pos)) /*!< OPAMP inverting input secondary connected to \
                                GPIO pin (pin PC5 for OPAMP1, pin PC5 for      \
                                OPAMP2, pin PB10  for OPAMP3, pin PB10 for     \
                                OPAMP4). Note: OPAMP inverting input is used   \
                                with OPAMP in mode standalone or PGA with      \
                                external capacitors for filtering circuit.     \
                                Otherwise (OPAMP in mode follower), OPAMP      \
                                inverting input is not used (not connected to  \
                                GPIO pin). */
#define LL_OPAMP_INPUT_INVERT_IO1_SEC                                          \
  (LL_OPAMP_INPUT_INVERT_IO1                                                   \
   << (OPAMP_CSR_VMSSEL_Pos -                                                  \
       OPAMP_CSR_VMSEL_Pos)) /*!< OPAMP inverting input secondary connected to \
                                GPIO pin (pin PA3 for OPAMP1, pin PA5 for      \
                                OPAMP2, pin PB2   for OPAMP3, pin PD8  for     \
                                OPAMP4). Note: OPAMP inverting input is used   \
                                with OPAMP in mode standalone or PGA with      \
                                external capacitors for filtering circuit.     \
                                Otherwise (OPAMP in mode follower), OPAMP      \
...
#define LL_OPAMP_INPUT_MUX_DISABLE \
  (0x00000000U) /*!< OPAMP inputs multiplexer mode dosabled. */
#define LL_OPAMP_INPUT_MUX_TIM1_CH6                                           \
  (OPAMP_CSR_TCMEN) /*!< OPAMP inputs multiplexer mode enabled, controlled by \
                       TIM1 CC6. */
/**
 * @}
 */

/** @defgroup OPAMP_LL_EC_VREF_OUTPUT OPAMP internal reference voltage path
 * state to output
 * @{
 */
#define LL_OPAMP_VREF_OUTPUT_DISABLE                                    \
  (0x00000000U) /*!< OPAMP internal reference voltage path to output is \
                   disabled. */
#define LL_OPAMP_VREF_OUTPUT_ENABLE                                          \
  (OPAMP_CSR_TSTREF) /*!< OPAMP internal reference voltage path to output is \
                        enabled. */
/**
 * @}
 */
...
#define LL_OPAMP_TRIMMING_NMOS_VREF_90PC_VDDA                                 \
  (OPAMP_CSR_TRIMOFFSETN | OPAMP_CSR_CALSEL_1 |                               \
   OPAMP_CSR_CALSEL_0) /*!< OPAMP trimming of transistors differential pair   \
                          ...
                          Default parameters to be used for calibration using \
                          two trimming steps (one with each transistors       \
...
#define LL_OPAMP_TRIMMING_PMOS_VREF_3_3PC_VDDA                                 \
  (OPAMP_CSR_TRIMOFFSETP) /*!< OPAMP trimming of transistors differential pair \
                             PMOS (internal reference voltage set to           \
                             0.33*Vdda). */
 ...
 * @param  OPAMPx OPAMP instance
 * @param  CalibrationVrefOutput This parameter can be one of the following
 * values:
 *         @arg @ref LL_OPAMP_VREF_OUTPUT_DISABLE
 *         @arg @ref LL_OPAMP_VREF_OUTPUT_ENABLE
 * @retval None
 */
__STATIC_INLINE void LL_OPAMP_SetCalibrationVrefOutput(
    OPAMP_TypeDef *OPAMPx, uint32_t CalibrationVrefOutput) {
 ...
 */
__STATIC_INLINE uint32_t
LL_OPAMP_GetCalibrationVrefOutput(OPAMP_TypeDef *OPAMPx) {
...
LL_OPAMP_IsCalibrationOutputSet(OPAMP_TypeDef *OPAMPx) {
  return ((READ_BIT(OPAMPx->CSR, OPAMP_CSR_OUTCAL) == OPAMP_CSR_OUTCAL) ? 1UL
                                                                        : 0UL);
}
...
LL_OPAMP_GetTrimmingValue(OPAMP_TypeDef *OPAMPx, uint32_t TransistorsDiffPair) {
  return (uint32_t)(READ_BIT(OPAMPx->CSR, (TransistorsDiffPair &
...
__STATIC_INLINE uint32_t LL_OPAMP_IsLocked(OPAMP_TypeDef *OPAMPx) {
  return ((READ_BIT(OPAMPx->CSR, OPAMP_CSR_LOCK) == (OPAMP_CSR_LOCK)) ? 1UL
                                                                      : 0UL);
}
