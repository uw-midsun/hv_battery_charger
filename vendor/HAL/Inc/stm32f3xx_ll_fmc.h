...
typedef struct {
  ...
  uint32_t
      WrapMode; /*!< Enables or disables the Wrapped burst access mode for Flash
...
#define __FMC_NAND_ENABLE(__INSTANCE__, __BANK__)                            \
  (((__BANK__) == FMC_NAND_BANK2) ? ((__INSTANCE__)->PCR2 |= FMC_PCRx_PBKEN) \
...
#define __FMC_NAND_DISABLE(__INSTANCE__, __BANK__)       \
  (((__BANK__) == FMC_NAND_BANK2)                        \
       ? CLEAR_BIT((__INSTANCE__)->PCR2, FMC_PCRx_PBKEN) \
