...
typedef enum {
  HAL_PCCARD_STATE_RESET =
      0x00U, /*!< PCCARD peripheral not yet initialized or disabled */
...
#endif /* USE_HAL_PCCARD_REGISTER_CALLBACKS  */
{
  ...
  FMC_PCCARD_InitTypeDef
      Init; /*!< PCCARD device control configuration parameters   */
  ...
  void (*MspDeInitCallback)(struct __PCCARD_HandleTypeDef
                                *hpccard); /*!< PCCARD Msp DeInit callback */
  void (*ItCallback)(
      struct __PCCARD_HandleTypeDef *hpccard); /*!< PCCARD IT callback */
#endif
...
#define PCCARD_ATTRIBUTE_SPACE_ADDRESS \
  0x98000000U /* Attribute space size to @0x9BFF FFFF */
#define PCCARD_COMMON_SPACE_ADDRESS \
  PCCARD_DEVICE_ADDRESS /* Common space size to @0x93FF FFFF    */
#define PCCARD_IO_SPACE_ADDRESS \
  0x9C000000U /* IO space size to @0x9FFF FFFF        */
#define PCCARD_IO_SPACE_PRIMARY_ADDR \
  0x9C0001F0U /* IO space size to @0x9FFF FFFF        */
...
#define ATA_STATUS_CMD \
  ((uint8_t)0x07) /* Status(read)/Command(write) register */
#define ATA_STATUS_CMD_ALTERNATE \
  ((uint8_t)0x0E) /* Alternate Status(read)/Command(write) register */
#define ATA_COMMON_DATA_AREA \
  ((uint16_t)0x0400) /* Start of data area (for Common access only!) */
#define ATA_CARD_CONFIGURATION \
  ((uint16_t)0x0202) /* Card Configuration and Status Register */
