...
#define LL_RTC_ALARM_OUTPUTTYPE_PUSHPULL                                   \
  RTC_TAFCR_ALARMOUTTYPE /*!< RTC_ALARM, when mapped on PC13, is push-pull \
                            output */
/**
 * @}
 */
...
#define LL_RTC_PIN_PC13                                                 \
  RTC_TAFCR_PC13MODE /*!< PC13 is forced to push-pull output if all RTC \
                        alternate functions are disabled */
#define LL_RTC_PIN_PC14                                                \
  RTC_TAFCR_PC14MODE /*!< PC14 is forced to push-pull output if LSE is \
                        disabled */
#define LL_RTC_PIN_PC15                                                \
  RTC_TAFCR_PC15MODE /*!< PC15 is forced to push-pull output if LSE is \
                        disabled */
/**
 * @}
 */
...
#define LL_RTC_TAMPER_DURATION_2RTCCLK                                         \
  RTC_TAFCR_TAMPPRCH_0 /*!< Tamper pins are pre-charged before sampling during \
                          2 RTCCLK cycles */
#define LL_RTC_TAMPER_DURATION_4RTCCLK                                         \
  RTC_TAFCR_TAMPPRCH_1 /*!< Tamper pins are pre-charged before sampling during \
                          4 RTCCLK cycles */
#define LL_RTC_TAMPER_DURATION_8RTCCLK                                         \
  RTC_TAFCR_TAMPPRCH /*!< Tamper pins are pre-charged before sampling during 8 \
                        RTCCLK cycles */
/**
 * @}
 */
...
#define LL_RTC_TAMPER_FILTER_2SAMPLE                                          \
  RTC_TAFCR_TAMPFLT_0 /*!< Tamper is activated after 2 consecutive samples at \
                         the active level */
#define LL_RTC_TAMPER_FILTER_4SAMPLE                                          \
  RTC_TAFCR_TAMPFLT_1 /*!< Tamper is activated after 4 consecutive samples at \
                         the active level */
#define LL_RTC_TAMPER_FILTER_8SAMPLE                                        \
  RTC_TAFCR_TAMPFLT /*!< Tamper is activated after 8 consecutive samples at \
                       the active level. */
/**
 * @}
 */
...
#define LL_RTC_TAMPER_SAMPLFREQDIV_2048                                  \
  RTC_TAFCR_TAMPFREQ_2 /*!< Each of the tamper inputs are sampled with a \
...
#define LL_RTC_TAMPER_SAMPLFREQDIV_256                                 \
  RTC_TAFCR_TAMPFREQ /*!< Each of the tamper inputs are sampled with a \
...
#define LL_RTC_TAMPER_ACTIVELEVEL_TAMP1                                     \
  RTC_TAFCR_TAMP1TRG /*!< RTC_TAMP1 input falling edge (if TAMPFLT = 00) or \
                        staying high (if TAMPFLT != 00) triggers a tamper   \
                        detection event */
#define LL_RTC_TAMPER_ACTIVELEVEL_TAMP2                                     \
  RTC_TAFCR_TAMP2TRG /*!< RTC_TAMP2 input falling edge (if TAMPFLT = 00) or \
                        staying high (if TAMPFLT != 00) triggers a tamper   \
                        detection event */
#if defined(RTC_TAMPER3_SUPPORT)
#define LL_RTC_TAMPER_ACTIVELEVEL_TAMP3                                     \
  RTC_TAFCR_TAMP3TRG /*!< RTC_TAMP3 input falling edge (if TAMPFLT = 00) or \
                        staying high (if TAMPFLT != 00) triggers a tamper   \
                        detection event */
#endif               /* RTC_TAMPER3_SUPPORT */
/**
 ...
 * @note   If all RTC alternate functions are disabled and PC13MODE = 1,
 ...
 * @note   PC13 forced to push-pull output if all RTC alternate functions are
 * disabled
 ...
 * @note   PC13, PC14 and/or PC15 are controlled by the GPIO configuration
 * registers. Consequently PC13, PC14 and/or PC15 are floating in Standby mode.
 ...
 * @note   Output data configuration is possible if the LSE is disabled and
 * PushPull output is enabled (through @ref LL_RTC_EnablePushPullMode)
...
__STATIC_INLINE uint32_t LL_RTC_TIME_Get(RTC_TypeDef *RTCx) {
  return (uint32_t)(READ_BIT(RTCx->TR, (RTC_TR_HT | RTC_TR_HU | RTC_TR_MNT |
 ...
 * @note   Bit is write-protected. @ref LL_RTC_DisableWriteProtection function
 * should be called before.
 * @note   Bit can be written only when RTC_CR WUTE bit = 0 and RTC_ISR WUTWF
 * bit = 1
