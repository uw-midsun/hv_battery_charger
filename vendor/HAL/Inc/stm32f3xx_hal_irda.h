...
typedef struct {
  uint32_t BaudRate; /*!< This member configures the IRDA communication baud
                        ...
                        following formula: Baud Rate Register = ((usart_ker_clk)
                        / ((hirda->Init.BaudRate))) where usart_ker_clk is the
                        IRDA input clock */
...
#define IRDA_FLAG_TXE \
  USART_ISR_TXE /*!< IRDA transmit data register empty         */
#define IRDA_FLAG_TC \
  USART_ISR_TC /*!< IRDA transmission complete                */
#define IRDA_FLAG_RXNE \
  USART_ISR_RXNE /*!< IRDA read data register not empty         */
