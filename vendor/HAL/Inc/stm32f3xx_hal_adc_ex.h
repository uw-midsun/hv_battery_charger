 ...
 * behaviour in case of intended action to update another parameter (which
 * fulfills the ADC state condition) on the fly).
 */
typedef struct {
  uint32_t
      ClockPrescaler; /*!< Select ADC clock source (synchronous clock derived
                         from AHB clock or asynchronous clock derived from ADC
                         dedicated PLL 72MHz) and clock prescaler. The clock is
                         common for all the ADCs. This parameter can be a value
                         of @ref ADCEx_ClockPrescaler Note: In case of usage of
                         channels on injected group, ADC frequency should be
                         lower than AHB clock frequency /4 for resolution 12 or
                         10 bits, AHB clock frequency /3 for resolution 8 bits,
                         AHB clock frequency /2 for resolution 6 bits. Note: In
                         case of usage of the ADC dedicated PLL clock, this
                         clock must be preliminarily enabled and prescaler set
                         at RCC top level. Note: This parameter can be modified
                         only if all ADCs of the common ADC group are disabled
  ...
  uint32_t
      DataAlign; /*!< Specifies ADC data alignment to right (for resolution 12
                    bits: MSB on register bit 11 and LSB on register bit 0U)
                    (default setting) or to left (for resolution 12 bits, if
                    offset disabled: MSB on register bit 15 and LSB on register
                    bit 4U, if offset enabled: MSB on register bit 14 and LSB on
                    register bit 3U). See reference manual for alignments with
                    other resolutions. This parameter can be a value of @ref
                    ADCEx_Data_align */
  uint32_t
      ScanConvMode; /*!< Configures the sequencer of regular and injected
                       ...
                       rank 1U). Parameters 'NbrOfConversion' and
                       'InjectedNbrOfConversion' are discarded (equivalent to
                       set to 1U). If enabled:  Conversions are performed in
                       ...
                       rank 'n'. This parameter can be a value of @ref
                       ADCEx_Scan_mode */
  uint32_t
      EOCSelection; /*!< Specifies what EOC (End Of Conversion) flag is used for
                       ...
                       a value of @ref ADCEx_EOCSelection. */
  FunctionalState
      LowPowerAutoWait; /*!< Selects the dynamic low power Auto Delay: ADC
                           conversions are performed only when necessary. New
                           conversion starts only when the previous conversion
                           (for regular group) or previous sequence (for
                           injected group) has been treated by user software.
                             This feature automatically adapts the speed of ADC
                           ...
                           data is needed: and use HAL_ADC_GetValue() to
                           retrieve conversion result and trig another
                           conversion (in case of usage of injected group, use
                           the equivalent functions HAL_ADCExInjected_Start(),
  ...
  uint32_t ExternalTrigConv; /*!< Selects the external event used to trigger the
                                ...
                                ADCEx_External_trigger_source_Regular Caution:
                                For devices with several ADCs, external trigger
                                source is common to ADC common group (for
  ...
  uint32_t ExternalTrigConvEdge; /*!< Selects the external trigger edge of
                                    ...
                                    @ref ADCEx_External_trigger_edge_Regular */
  FunctionalState
      DMAContinuousRequests; /*!< Specifies whether the DMA requests are
                                ...
                                reached. This parameter can be set to ENABLE or
                                DISABLE. Note: This parameter must be modified
  ...
  uint32_t
      Overrun; /*!< Select the behaviour in case of overrun: data overwritten
                  (default) or preserved. This parameter is for regular group
                  only. This parameter can be a value of @ref ADCEx_Overrun
                    Note: Case of overrun set to data preserved and usage with
                  end on conversion interruption (HAL_Start_IT()): ADC IRQ
                  handler has to clear end of conversion flags, this induces the
                  release of the preserved data. If needed, this data can be
                  saved into function HAL_ADC_ConvCpltCallback() (called before
                  end of conversion flags clear). Note: Error reporting in
                  function of conversion mode:
                  ...
                  willingly not read the conversion data each time, this is not
                  ...
                  from data register, any data missed would be abnormal). */
} ADC_InitTypeDef;
 ...
 */
typedef struct {
  uint32_t Channel; /*!< Specifies the channel to configure into ADC regular
                       group. This parameter can be a value of @ref
                       ADCEx_channels Note: Depending on devices, some channels
                       may not be available on package pins. Refer to device
                       datasheet for channels availability. */
  uint32_t Rank;    /*!< Specifies the rank in the regular group sequencer.
                         This parameter can be a value of @ref ADCEx_regular_rank
                         Note: In case of need to disable a channel or change order
                       of conversion sequencer, rank containing a previous channel
                       setting can be overwritten by the new channel setting (or
  ...
  uint32_t
      SamplingTime; /*!< Sampling time value to be set for the selected channel.
                       ...
                       value of @ref ADCEx_sampling_times Caution: This
                       parameter updates the parameter property of the channel,
                       that can be used into regular and/or injected groups. If
                       this same channel has been previously configured in the
                       other group (regular/injected), it will be updated to
                       last setting. Note: In case of usage of internal
                       measurement channels (VrefInt/Vbat/TempSensor), sampling
                       time constraints must be respected (sampling time can be
                       adjusted in function of ADC clock frequency and sampling
                       time setting) Refer to device datasheet for timings
                       values, parameters TS_vrefint, TS_vbat, TS_temp (values
                       rough order: 2.2us min). */
  uint32_t
      SingleDiff; /*!< Selection of single-ended or differential input.
                     ...
                     must be a value of @ref ADCEx_SingleDifferential Caution:
                     This parameter updates the parameter property of the
                     channel, that can be used into regular and/or injected
                     groups. If this same channel has been previously configured
                     in the other group (regular/injected), it will be updated
                     to last setting. Note: Channels 1 to 14 are available in
                     differential mode. Channels 15U, 16U, 17U, 18 can be used
                     only in single-ended mode. Note: When configuring a channel
                     'i' in differential mode, the channel 'i+1' is not usable
                     separately. Note: This parameter must be modified when ADC
                     is disabled (before ADC start conversion or after ADC stop
                   ...
                   */
  uint32_t
      OffsetNumber; /*!< Selects the offset number
                         This parameter can be a value of @ref
                       ADCEx_OffsetNumber Caution: Only one channel is allowed
                       per channel. If another channel was on this offset
                       number, the offset will be changed to the new channel */
  uint32_t
      Offset; /*!< Defines the offset to be subtracted from the raw converted
                 data when convert channels. Offset value must be a positive
                 number. Depending of ADC resolution selected (12U, 10U, 8 or 6
                 bits), this parameter must be a number between Min_Data = 0x000
                 and Max_Data = 0xFFFU, 0x3FFU, 0xFF or 0x3F respectively. Note:
 ...
 * QueueInjectedContext, ExternalTrigInjecConvEdge, ExternalTrigInjecConv.
 ...
 * regular and injected groups. If ADC is not in the appropriate state to modify
 * some parameters, these parameters setting is bypassed without error reporting
 * (as it can be the expected behaviour in case of intended action to update
...
typedef struct {
  ...
  uint32_t
      InjectedSamplingTime; /*!< Sampling time value to be set for the selected
                               ...
                               be a value of @ref ADCEx_sampling_times Caution:
                               This parameter updates the parameter property of
                               the channel, that can be used into regular and/or
  ...
  uint32_t
      InjectedSingleDiff; /*!< Selection of single-ended or differential input.
                             ...
                             value of @ref ADCEx_SingleDifferential Caution:
                             This parameter updates the parameter property of
                             the channel, that can be used into regular and/or
                             injected groups. If this same channel has been
                             previously configured in the other group
                             (regular/injected), it will be updated to last
                             setting. Note: Channels 1 to 14 are available in
                             differential mode. Channels 15U, 16U, 17U, 18 can
                             be used only in single-ended mode. Note: When
                             ...
                             can be the expected behaviour in case of another
                             parameter update on the fly) */
  uint32_t
      InjectedOffsetNumber; /*!< Selects the offset number
                                 This parameter can be a value of @ref
                               ADCEx_OffsetNumber Caution: Only one channel is
                               allowed per offset number. If another channel was
                               on this offset number, the offset will be changed
                               to the new channel. */
  uint32_t InjectedOffset; /*!< Defines the offset to be subtracted from the raw
                              converted data. Offset value must be a positive
                              number. Depending of ADC resolution selected (12U,
                              10U, 8 or 6 bits), this parameter must be a number
  ...
  FunctionalState
      QueueInjectedContext; /*!< Specifies whether the context queue feature is
                               ...
                               start. Therefore, configure channels with
                               HAL_ADCEx_InjectedConfigChannel() as many times
                               as value of 'InjectedNbrOfConversion' parameter.
  ...
  uint32_t
      ExternalTrigInjecConv; /*!< Selects the external event used to trigger the
                                ...
                                are disabled. This parameter can be a value of
                                @ref ADCEx_External_trigger_source_Injected
                                  Caution: this setting impacts the entire
                                injected group. Therefore, call of
                                HAL_ADCEx_InjectedConfigChannel() to configure a
                                channel on injected group can impact the
 ...
 * either: ADC disabled or ADC enabled without conversion on going on regular
 * and injected groups.
 */
typedef struct {
  uint32_t
      WatchdogNumber; /*!< Selects which ADC analog watchdog to apply to the
  ...
  uint32_t
      WatchdogMode; /*!< For Analog Watchdog 1: Configures the ADC analog
                       watchdog mode: single channel/overall group of channels,
                       regular/injected group. For Analog Watchdog 2 and 3:
                       There is no configuration for overall group of channels
                       as AWD1. Set value 'ADC_ANALOGWATCHDOG_NONE' to reset
                       channels group programmed with parameter 'Channel', set
                       any other value to not use this parameter. This parameter
                       can be a value of @ref ADCEx_analog_watchdog_mode. */
  uint32_t
      Channel; /*!< Selects which ADC channel to monitor by analog watchdog.
                  ...
                  Several channels can be monitored (successive calls of
                  HAL_ADC_AnalogWDGConfig() must be done, one for each channel.
  ...
  uint32_t
      HighThreshold; /*!< Configures the ADC analog watchdog High threshold
                        value. Depending of ADC resolution selected (12U, 10U, 8
                        or 6 bits), this parameter must be a number between
                        Min_Data = 0x000 and Max_Data = 0xFFFU, 0x3FFU, 0xFF or
 ...
 * the common group). ADC state can be either:
 *          - For all parameters: ADC disabled (this is the only possible ADC
 ...
 * it can be the expected behaviour in case of intended action to update another
...
typedef struct {
  ...
  uint32_t TwoSamplingDelay; /*!< Configures the Delay between 2 sampling
                                ...
                                range depends on selected resolution: from 1 to
                                12 clock cycles for 12 bits, from 1 to 10 clock
 ...
 * ExternalTrigConv.
 * @note   The setting of these parameters with function HAL_ADC_Init() is
 * conditioned to ADC state. ADC can be either disabled or enabled without
 * conversion on going on regular group.
 */
typedef struct {
  uint32_t
      DataAlign; /*!< Specifies ADC data alignment to right (MSB on register bit
                    11 and LSB on register bit 0U) (default setting) or to left
                    (if regular group: MSB on register bit 15 and LSB on
                    register bit 4U, if injected group (MSB kept as signed value
                    due to potential negative value after offset application):
                    MSB on register bit 14 and LSB on register bit 3U). This
                    parameter can be a value of @ref ADCEx_Data_align */
  uint32_t
      ScanConvMode; /*!< Configures the sequencer of regular and injected
                       groups. This parameter can be associated to parameter
                       'DiscontinuousConvMode' to have main sequence subdivided
                       ...
                       channel rank). Scan direction is upward: from rank1 to
                       rank 'n'. This parameter can be a value of @ref
                       ADCEx_Scan_mode Note: For regular group, this parameter
                       should be enabled in conversion either by polling
                       (HAL_ADC_Start with Discontinuous mode and
                       NbrOfDiscConversion=1U) or by DMA (HAL_ADC_Start_DMA),
                       but not by interruption (HAL_ADC_Start_IT): in scan mode,
                       interruption is triggered only on the the last conversion
                       of the sequence. All previous conversions would be
                       overwritten by the last one. Injected group used with
                       scan mode has not this constraint: each rank has its own
                       result register, no data is overwritten. */
  FunctionalState
      ContinuousConvMode; /*!< Specifies whether the conversion is performed in
                             single mode (one conversion) or continuous mode for
                             regular group, after the selected trigger occurred
  ...
  uint32_t
      ExternalTrigConv; /*!< Selects the external event used to trigger the
                           ...
                           event rising edge. This parameter can be a value of
                           @ref ADCEx_External_trigger_source_Regular */
} ADC_InitTypeDef;
...
typedef struct {
  ...
  uint32_t
      SamplingTime; /*!< Sampling time value to be set for the selected channel.
                         ...
                         Conversion time is the addition of sampling time and
                       processing time (12.5 ADC clock cycles at ADC resolution
                       12 bits). This parameter can be a value of @ref
                       ADCEx_sampling_times Caution: This parameter updates the
                       ...
                       TS_vrefint, TS_vbat, TS_temp (values rough order: 5us
                       to 17.1us min). */
} ADC_ChannelConfTypeDef;
...
typedef struct {
  ...
  uint32_t
      InjectedSamplingTime; /*!< Sampling time value to be set for the selected
                               ...
                               is the addition of sampling time and processing
                               time (12.5 ADC clock cycles at ADC resolution 12
                               bits). This parameter can be a value of @ref
                               ADCEx_sampling_times Caution: This parameter
                               updates the parameter property of the channel,
                               that can be used into regular and/or injected
                               groups. If this same channel has been previously
                               configured in the other group (regular/injected),
                               it will be updated to last setting. Note: In case
                               of usage of internal measurement channels
                               ...
                               datasheet for timings values, parameters
                               TS_vrefint, TS_vbat, TS_temp (values rough order:
                               5us to 17.1us min). */
  uint32_t InjectedOffset; /*!< Defines the offset to be subtracted from the raw
 ...
 * state. ADC state can be either disabled or enabled without conversion on
 * going on regular and injected groups.
 */
typedef struct {
  uint32_t
      WatchdogMode; /*!< Configures the ADC analog watchdog mode: single/all
                       channels, regular/injected group. This parameter can be a
                       value of @ref ADCEx_analog_watchdog_mode. */
  uint32_t
      Channel; /*!< Selects which ADC channel to monitor by analog watchdog.
                    This parameter has an effect only if watchdog mode is
                  configured on single channel (parameter WatchdogMode) This
                  parameter can be a value of @ref ADCEx_channels. */
  FunctionalState ITMode; /*!< Specifies whether the analog watchdog is
                             configured in interrupt or polling mode. This
                             parameter can be set to ENABLE or DISABLE */
...
#define ADC_CLOCK_SYNC_PCLK_DIV1                                             \
  ((uint32_t)ADC12_CCR_CKMODE_0) /*!< ADC synchronous clock derived from AHB \
                                    clock without prescaler */
#define ADC_CLOCK_SYNC_PCLK_DIV2                                             \
  ((uint32_t)ADC12_CCR_CKMODE_1) /*!< ADC synchronous clock derived from AHB \
                                    clock divided by a prescaler of 2U */
...
#define ADC_CLOCK_SYNC_PCLK_DIV1                                            \
  ((uint32_t)ADC1_CCR_CKMODE_0) /*!< ADC synchronous clock derived from AHB \
                                   clock without prescaler */
#define ADC_CLOCK_SYNC_PCLK_DIV2                                            \
  ((uint32_t)ADC1_CCR_CKMODE_1) /*!< ADC synchronous clock derived from AHB \
                                   clock divided by a prescaler of 2U */
#define ADC_CLOCK_SYNC_PCLK_DIV4                                          \
...
#define ADC_OVR_DATA_OVERWRITTEN                                              \
  (0x00000000U) /*!< Default setting, to be used for compatibility with other \
                   STM32 devices */
...
#define ADC_DMAACCESSMODE_12_10_BITS                                           \
  ((uint32_t)ADC12_CCR_MDMA_1) /*!< DMA multimode enabled (one DMA channel for \
                                  both ADC, DMA of ADC master) for 12 and 10   \
                                  bits resolution */
#define ADC_DMAACCESSMODE_8_6_BITS                                            \
  ((uint32_t)ADC12_CCR_MDMA) /*!< DMA multimode enabled (one DMA channel for  \
...
#define ADC_AWD2_EVENT                                                    \
  ((uint32_t)                                                             \
       ADC_FLAG_AWD2) /*!< ADC Analog watchdog 2 event (additional analog \
                         watchdog, not present on all STM32 families) */
...
#define ADC_IT_EOS                                                             \
  ADC_IER_EOS /*!< ADC End of Regular sequence of Conversions interrupt source \
               */
...
#define ADC_IT_AWD1                                                     \
  ADC_IER_AWD1 /*!< ADC Analog watchdog 1 interrupt source (main analog \
                  watchdog, present on all STM32 devices) */
#define ADC_IT_AWD2                                                           \
  ADC_IER_AWD2 /*!< ADC Analog watchdog 2 interrupt source (additional analog \
                  watchdog, present only on STM32F3 devices) */
#define ADC_IT_AWD3                                                           \
  ADC_IER_AWD3 /*!< ADC Analog watchdog 3 interrupt source (additional analog \
                  watchdog, present only on STM32F3 devices) */
#define ADC_IT_JQOVF \
  ADC_IER_JQOVF /*!< ADC Injected Context Queue Overflow interrupt source */

#define ADC_IT_AWD                                                            \
  ADC_IT_AWD1 /* ADC Analog watchdog 1 interrupt source: Alternate naming for \
                 compatibility with other STM32 devices having only 1 analog  \
                 watchdog */
/**
 * @}
 */
...
#define ADC_FLAG_AWD1                                                         \
  ADC_ISR_AWD1 /*!< ADC Analog watchdog 1 flag (main analog watchdog, present \
...
#define ADC1_EXTERNALTRIG_T2_CC2 \
  ((uint32_t)(ADC_CFGR_EXTSEL_1 | ADC_CFGR_EXTSEL_0))
 ...
 * watchdog, present on all STM32 devices)
 *            @arg ADC_FLAG_AWD2:  ADC Analog watchdog 2 flag (additional analog
 * watchdog, present only on STM32F3 devices)
 *            @arg ADC_FLAG_AWD3:  ADC Analog watchdog 3 flag (additional analog
 * watchdog, present only on STM32F3 devices)
 ...
 * to 0 If resolution 12 bits, no shift. If resolution 10 bits, shift of 2 ranks
 * on the left. If resolution 8 bits, shift of 4 ranks on the left. If
...
#define ADC_AWD1THRESHOLD_SHIFT_RESOLUTION(__HANDLE__, _Threshold_)          \
  ((_Threshold_) << ((((__HANDLE__)->Instance->CFGR & ADC_CFGR_RES) >> 3U) * \
                     2U))

/**
 * @brief Shift the AWD2 and AWD3 threshold in function of the selected ADC
 * resolution. Thresholds have to be left-aligned on bit 7. If resolution 12
 * bits, shift of 4 ranks on the right (the 4 LSB are discarded) If resolution
 * 10 bits, shift of 2 ranks on the right (the 2 LSB are discarded) If
 * resolution 8 bits, no shift. If resolution 6 bits, shift of 2 ranks on the
 * left (the 2 LSB are set to 0)
 * @param __HANDLE__ ADC handle
 * @param _Threshold_ Value to be shifted
 * @retval None
 */
#define ADC_AWD23THRESHOLD_SHIFT_RESOLUTION(__HANDLE__, _Threshold_)           \
  (((__HANDLE__)->Instance->CFGR & ADC_CFGR_RES) !=                            \
       ...
       ? ((_Threshold_) >>                                                     \
          (4U - ((((__HANDLE__)->Instance->CFGR & ADC_CFGR_RES) >> 3U) * 2U))) \
       : (_Threshold_) << 2U)

/**
 * @brief Defines if the selected ADC is within ADC common register ADC1_2 or
 * ADC3_4 if available (ADC2, ADC3, ADC4 availability depends on STM32 product)
 * @param __HANDLE__ ADC handle
...
/**
 * @brief Verification of condition for ADC group injected start conversion: ADC
 * must be in non-multimode or multimode on group regular only, or multimode
 * with handle of ADC master (applicable for devices with several ADCs)
 * @param __HANDLE__ ADC handle.
 * @retval None
 */
...
/**
 * @brief Set handle of the other ADC sharing the same common register ADC1_2 or
 * ADC3_4 if available (ADC2, ADC3, ADC4 availability depends on STM32 product)
 * @param __HANDLE__ ADC handle
...
#define IS_ADC_CHANNEL(CHANNEL)                                                \
  (((CHANNEL) == ADC_CHANNEL_1) || ((CHANNEL) == ADC_CHANNEL_2) ||             \
   ...
   ((CHANNEL) == ADC_CHANNEL_VOPAMP1) || ((CHANNEL) == ADC_CHANNEL_VOPAMP2) || \
...
#define IS_ADC_SINGLE_DIFFERENTIAL(SING_DIFF) \
  (((SING_DIFF) == ADC_SINGLE_ENDED) || ((SING_DIFF) == ADC_DIFFERENTIAL_ENDED))
...
#define ADC_CONVCYCLES_MAX_RANGE(__HANDLE__)                                   \
  (((((__HANDLE__)->Instance->SMPR2 & ADC_SAMPLETIME_ALLCHANNELS_SMPR2BIT2) == \
     RESET) &&                                                                 \
    (((__HANDLE__)->Instance->SMPR1 & ADC_SAMPLETIME_ALLCHANNELS_SMPR1BIT2) == \
     RESET))                                                                   \
       ?                                                                       \
                                                                               \
