...
#define RCC_PLLSOURCE_HSI                                                      \
  RCC_CFGR_PLLSRC_HSI_PREDIV /*!< HSI clock selected as PLL entry clock source \
                              */
...
#define RCC_FLAG_LSERDY                                              \
  ((uint8_t)((BDCR_REG_INDEX << 5U) |                                \
             POSITION_VAL(RCC_BDCR_LSERDY))) /*!< External Low Speed \
                                                oscillator Ready */

/* Flags in the CFGR register */
#if defined(RCC_CFGR_MCOF)
#define RCC_FLAG_MCO                  \
  ((uint8_t)((CFGR_REG_INDEX << 5U) | \
             POSITION_VAL(            \
 ...
 * RTC clock source, the RTC cannot be used in STOP and STANDBY modes.
 * @note   The system must always be configured so as to get a PCLK frequency
 * greater than or equal to the RTCCLK frequency for a proper operation of the
 * RTC.
 */
