/**
  ...
  ******************************************************************************
  ...
  power mode configuration requiring different wake up sources.
    ...
    (@) The stm32f3xx_hal_timebase_tim use is recommended for the
...
#error Please select the RTC Clock source
  ...
  if (status == HAL_OK) {
    ...
    /* The time base should be 1ms
       Time base = ((RTC_ASYNCH_PREDIV + 1U) * (RTC_SYNCH_PREDIV + 1U)) /
       RTC_CLOCK HSE/32 as RTC clock and HSE 8MHz Time base = ((49U + 1U) * (4U
       + 1U)) / 250kHz = 1ms LSE as RTC clock Time base = ((31U + 1U) * (0U +
       1U)) / 32.768KHz = ~1ms LSI as RTC clock Time base = ((39U + 1U) * (0U +
       1U)) / 40KHz = 1ms
    */
