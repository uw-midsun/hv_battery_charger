/**
  ...
  ******************************************************************************
  ...
  asynchronous static memories and 16-bit PC memory cards. Its main purposes
  are:
...
#define PCR_CLEAR_MASK                                                        \
  ((uint32_t)(FMC_PCRx_PWAITEN | FMC_PCRx_PBKEN | FMC_PCRx_PTYP |             \
              FMC_PCRx_PWID | FMC_PCRx_ECCEN | FMC_PCRx_TCLR | FMC_PCRx_TAR | \
              FMC_PCRx_ECCPS))
/* --- PMEM Register ---*/
/* PMEM register clear mask */
#define PMEM_CLEAR_MASK                                                     \
  ((uint32_t)(FMC_PMEMx_MEMSETx | FMC_PMEMx_MEMWAITx | FMC_PMEMx_MEMHOLDx | \
              FMC_PMEMx_MEMHIZx))

/* --- PATT Register ---*/
/* PATT register clear mask */
#define PATT_CLEAR_MASK                                                     \
  ((uint32_t)(FMC_PATTx_ATTSETx | FMC_PATTx_ATTWAITx | FMC_PATTx_ATTHOLDx | \
              FMC_PATTx_ATTHIZx))
...
#define PCR4_CLEAR_MASK                                                       \
  ((uint32_t)(FMC_PCR4_PWAITEN | FMC_PCR4_PBKEN | FMC_PCR4_PTYP |             \
              FMC_PCR4_PWID | FMC_PCR4_ECCEN | FMC_PCR4_TCLR | FMC_PCR4_TAR | \
              FMC_PCR4_ECCPS))
/* --- PMEM Register ---*/
/* PMEM register clear mask */
#define PMEM4_CLEAR_MASK                                                    \
  ((uint32_t)(FMC_PMEM4_MEMSET4 | FMC_PMEM4_MEMWAIT4 | FMC_PMEM4_MEMHOLD4 | \
              FMC_PMEM4_MEMHIZ4))

/* --- PATT Register ---*/
/* PATT register clear mask */
#define PATT4_CLEAR_MASK                                                    \
  ((uint32_t)(FMC_PATT4_ATTSET4 | FMC_PATT4_ATTWAIT4 | FMC_PATT4_ATTHOLD4 | \
              FMC_PATT4_ATTHIZ4))

/* --- PIO4 Register ---*/
/* PIO4 register clear mask */
#define PIO4_CLEAR_MASK                                               \
  ((uint32_t)(FMC_PIO4_IOSET4 | FMC_PIO4_IOWAIT4 | FMC_PIO4_IOHOLD4 | \
              FMC_PIO4_IOHIZ4))
...
HAL_StatusTypeDef FMC_NAND_CommonSpace_Timing_Init(
    FMC_NAND_TypeDef *Device, FMC_NAND_PCC_TimingTypeDef *Timing,
    uint32_t Bank) {
  /* Check the parameters */
  assert_param(IS_FMC_NAND_DEVICE(Device));
...
HAL_StatusTypeDef FMC_PCCARD_Init(FMC_PCCARD_TypeDef *Device,
  ...
  MODIFY_REG(Device->PCR4,
             (FMC_PCR4_PTYP | FMC_PCR4_PWAITEN | FMC_PCR4_PWID | FMC_PCR4_TCLR |
              FMC_PCR4_TAR),
             (FMC_PCR_MEMORY_TYPE_PCCARD | Init->Waitfeature |
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef FMC_PCCARD_CommonSpace_Timing_Init(
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef FMC_PCCARD_IOSpace_Timing_Init(
    FMC_PCCARD_TypeDef *Device, FMC_NAND_PCC_TimingTypeDef *Timing) {
  /* Check the parameters */
  assert_param(IS_FMC_PCCARD_DEVICE(Device));
#if defined(FMC_BANK3)
