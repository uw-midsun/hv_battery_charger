...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization
  ...
  *
  ...
  through the PLL as System clock source. The HSI clock can be used also to
  clock the USART and I2C peripherals.
  ...
  LSI, LSE or PLL clock (divided by 2) output on pin (such as PA8 pin).
  ...
  (SYSCLK) frequency must be higher or equal to the RTC clock frequency.
        (++) IWDG clock which is always the LSI clock.

         (#) For the STM32F3xx devices, the maximum frequency of the SYSCLK,
  HCLK, PCLK1 and PCLK2 is 72 MHz, Depending on the SYSCLK frequency, the flash
...
HAL_StatusTypeDef HAL_RCC_DeInit(void) {
  ...
  CLEAR_BIT(RCC->CFGR, RCC_CFGR_SW | RCC_CFGR_HPRE | RCC_CFGR_PPRE1 |
