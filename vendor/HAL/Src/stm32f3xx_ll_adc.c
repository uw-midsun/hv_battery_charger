...
#define ADC_TIMEOUT_STOP_CONVERSION_CPU_CYCLES \
  (ADC_CLOCK_RATIO_VS_CPU_HIGHEST * 1U)
  ...
  if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(ADCxy_COMMON) == 0U) {
    ...
    if (ADC_CommonInitStruct->Multimode != LL_ADC_MULTI_INDEPENDENT) {
      MODIFY_REG(ADCxy_COMMON->CCR,
                 ADC_CCR_CKMODE | ADC_CCR_DUAL | ADC_CCR_MDMA | ADC_CCR_DELAY,
...
ErrorStatus LL_ADC_DeInit(ADC_TypeDef *ADCx) {
  ...
  if (READ_BIT(ADCx->CR, (ADC_CR_JADSTP | ADC_CR_ADSTP | ADC_CR_JADSTART |
                          ADC_CR_ADSTART | ADC_CR_ADDIS | ADC_CR_ADEN)) == 0U) {
    /* ========== Reset ADC registers ========== */
    /* Reset register IER */
...
ErrorStatus LL_ADC_DeInit(ADC_TypeDef *ADCx) {
  ...
  if (READ_BIT(ADCx->CR2, ADC_CR2_ADON) == 0U) {
    ...
    /* Reset register SR */
    CLEAR_BIT(ADCx->SR,
              (LL_ADC_FLAG_STRT | LL_ADC_FLAG_JSTRT | LL_ADC_FLAG_EOS |
               LL_ADC_FLAG_JEOS | LL_ADC_FLAG_AWD1));

    /* Reset register CR1 */
    CLEAR_BIT(ADCx->CR1, (ADC_CR1_AWDEN | ADC_CR1_JAWDEN | ADC_CR1_DISCNUM |
    ...
    /* Reset register CR2 */
    CLEAR_BIT(ADCx->CR2,
              (ADC_CR2_TSVREFE | ADC_CR2_SWSTART | ADC_CR2_EXTTRIG |
               ADC_CR2_EXTSEL | ADC_CR2_JSWSTART | ADC_CR2_JEXTTRIG |
               ADC_CR2_JEXTSEL | ADC_CR2_ALIGN | ADC_CR2_DMA | ADC_CR2_RSTCAL |
               ADC_CR2_CAL | ADC_CR2_CONT | ADC_CR2_ADON));

    /* Reset register SMPR1 */
    CLEAR_BIT(ADCx->SMPR1,
              (ADC_SMPR1_SMP17 | ADC_SMPR1_SMP16 | ADC_SMPR1_SMP15 |
...
ErrorStatus LL_ADC_REG_Init(ADC_TypeDef *ADCx,
  ...
  if (LL_ADC_IsEnabled(ADCx) == 0U) {
    ...
    if (ADC_REG_InitStruct->SequencerLength != LL_ADC_REG_SEQ_SCAN_DISABLE) {
      ...
      MODIFY_REG(
          ADCx->CR1, ADC_CR1_DISCEN | ADC_CR1_DISCNUM,
