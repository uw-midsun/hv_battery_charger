...
#elif defined(UART4)
#define IS_LL_RCC_UART_INSTANCE(__VALUE__) \
  ((__VALUE__) == LL_RCC_UART4_CLKSOURCE)
#elif defined(UART5)
#define IS_LL_RCC_UART_INSTANCE(__VALUE__) \
  ((__VALUE__) == LL_RCC_UART5_CLKSOURCE)
...
#define IS_LL_RCC_ADC_CLKSOURCE(__VALUE__) \
  (((__VALUE__) == LL_RCC_ADC1_CLKSOURCE))
...
#if defined(RCC_CFGR_SDPRE)
#define IS_LL_RCC_SDADC_CLKSOURCE(__VALUE__) \
  (((__VALUE__) == LL_RCC_SDADC_CLKSOURCE))
#endif /* RCC_CFGR_SDPRE */
...
#if defined(RCC_CFGR3_TIM8SW) && defined(RCC_CFGR3_TIM15SW) &&  \
    defined(RCC_CFGR3_TIM16SW) && defined(RCC_CFGR3_TIM17SW) && \
    defined(RCC_CFGR3_TIM20SW) && defined(RCC_CFGR3_TIM2SW) &&  \
...
#elif !defined(RCC_CFGR3_TIM8SW) && defined(RCC_CFGR3_TIM15SW) && \
    defined(RCC_CFGR3_TIM16SW) && defined(RCC_CFGR3_TIM17SW) &&   \
    !defined(RCC_CFGR3_TIM20SW) && defined(RCC_CFGR3_TIM2SW) &&   \
    defined(RCC_CFGR3_TIM34SW)
...
#if defined(HRTIM1)
#define IS_LL_RCC_HRTIM_CLKSOURCE(__VALUE__) \
  (((__VALUE__) == LL_RCC_HRTIM1_CLKSOURCE))
#endif /* HRTIM1 */
 ...
 *         HSI_VALUE(**) or HSE_VALUE(***) multiplied/divided by the PLL
 * factors.
 ...
 *         @arg @ref LL_RCC_PERIPH_FREQUENCY_NO indicates that HSI oscillator is
 * not ready
 */
 ...
 *         @arg @ref LL_RCC_PERIPH_FREQUENCY_NA indicates that external clock is
 * used */
uint32_t LL_RCC_GetI2SClockFreq(uint32_t I2SxSource) {
  ...
  switch (LL_RCC_GetI2SClockSource(I2SxSource)) {
    case LL_RCC_I2S_CLKSOURCE_SYSCLK: /*!< System clock selected as I2S clock
                                         source */
      i2s_frequency = RCC_GetSystemClockFreq();
      break;

    /* If an external I2S clock has to be used,
       LL_RCC_SetI2SClockSource(LL_RCC_I2S_CLKSOURCE_PIN) have to be called in
       the main after calling SystemClock_Config() */
    case LL_RCC_I2S_CLKSOURCE_PIN: /*!< External clock selected as I2S clock
                                      source */
 ...
 *         @arg @ref LL_RCC_PERIPH_FREQUENCY_NO indicates that oscillator
 * (HSI48) or PLL is not ready
 *         @arg @ref LL_RCC_PERIPH_FREQUENCY_NA indicates that no clock source
 * selected
 */
...
uint32_t LL_RCC_GetADCClockFreq(uint32_t ADCxSource) {
  ...
  if ((adc_prescaler & 0x0000FFFFU) == ((uint32_t)0x00000000U)) {
    ...
    adc_frequency =
        RCC_GetPCLK2ClockFreq(RCC_GetHCLKClockFreq(RCC_GetSystemClockFreq())) /
        (aADCPrescTable[((adc_prescaler & 0x0000FFFFU) >>
...
uint32_t LL_RCC_GetSDADCClockFreq(uint32_t SDADCxSource) {
  ...
  sdadc_frequency =
      ...
      (aSDADCPrescTable[(sdadc_prescaler >> POSITION_VAL(SDADCxSource)) &
                        0xFU]);
 ...
 *        @arg @ref LL_RCC_PERIPH_FREQUENCY_NO indicates that oscillators (HSI
 * or LSE) are not ready
 */
uint32_t LL_RCC_GetCECClockFreq(uint32_t CECxSource) {
  ...
  switch (LL_RCC_GetCECClockSource(CECxSource)) {
    case LL_RCC_CEC_CLKSOURCE_HSI_DIV244: /* HSI / 244 clock used as CEC clock
                                             source */
      if (LL_RCC_HSI_IsReady()) {
...
uint32_t LL_RCC_GetTIMClockFreq(uint32_t TIMxSource) {
  ...
  if (TIMxSource == LL_RCC_TIM1_CLKSOURCE) {
    ...
    if (LL_RCC_GetTIMClockSource(LL_RCC_TIM1_CLKSOURCE) ==
  ...
  if (TIMxSource == LL_RCC_TIM8_CLKSOURCE) {
    ...
    if (LL_RCC_GetTIMClockSource(LL_RCC_TIM8_CLKSOURCE) ==
  ...
  if (TIMxSource == LL_RCC_TIM15_CLKSOURCE) {
    ...
    if (LL_RCC_GetTIMClockSource(LL_RCC_TIM15_CLKSOURCE) ==
  ...
  if (TIMxSource == LL_RCC_TIM16_CLKSOURCE) {
    ...
    if (LL_RCC_GetTIMClockSource(LL_RCC_TIM16_CLKSOURCE) ==
  ...
  if (TIMxSource == LL_RCC_TIM17_CLKSOURCE) {
    ...
    if (LL_RCC_GetTIMClockSource(LL_RCC_TIM17_CLKSOURCE) ==
  ...
  if (TIMxSource == LL_RCC_TIM20_CLKSOURCE) {
    ...
    if (LL_RCC_GetTIMClockSource(LL_RCC_TIM20_CLKSOURCE) ==
  ...
  if (TIMxSource == LL_RCC_TIM2_CLKSOURCE) {
    ...
    if (LL_RCC_GetTIMClockSource(LL_RCC_TIM2_CLKSOURCE) ==
  ...
  if (TIMxSource == LL_RCC_TIM34_CLKSOURCE) {
    ...
    if (LL_RCC_GetTIMClockSource(LL_RCC_TIM34_CLKSOURCE) ==
...
uint32_t LL_RCC_GetHRTIMClockFreq(uint32_t HRTIMxSource) {
  ...
  if (LL_RCC_GetHRTIMClockSource(LL_RCC_HRTIM1_CLKSOURCE) ==
      LL_RCC_HRTIM1_CLKSOURCE_PCLK2) {
...
uint32_t RCC_PLL_GetFreqDomain_SYS(void) {
  ...
  return __LL_RCC_CALC_PLLCLK_FREQ(
      (pllinputfreq / (LL_RCC_PLL_GetPrediv() + 1U)),
