/**
  ...
  ******************************************************************************
  ...
  clock or asynchronous clock derived from ADC dedicated PLL 72MHz.
              - Synchronous clock is mandatory since used as ADC core clock.
                Synchronous clock can be used optionally as ADC conversion
  clock, depending on ADC init structure clock setting. Synchronous clock is
  configured using macro __ADCx_CLK_ENABLE().
              - Asynchronous can be used optionally as ADC conversion clock,
  depending on ADC init structure clock setting. Asynchronous clock is
  configured using function HAL_RCCEx_PeriphCLKConfig().
 ...
 *          @arg @ref HAL_ADC_CONVERSION_HALF_CB_ID          ADC conversion
 * complete callback ID
 ...
 * @retval HAL status
 */
__weak HAL_StatusTypeDef HAL_ADC_PollForConversion(ADC_HandleTypeDef* hadc,
                                                   uint32_t Timeout) {
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hadc);
  UNUSED(Timeout);
 ...
 * @retval HAL status
 */
__weak HAL_StatusTypeDef HAL_ADC_PollForEvent(ADC_HandleTypeDef* hadc,
                                              uint32_t EventType,
                                              uint32_t Timeout) {
  /* Prevent unused argument(s) compilation warning */
  UNUSED(hadc);
 ...
 * @retval None
 */
__weak HAL_StatusTypeDef HAL_ADC_Start_DMA(ADC_HandleTypeDef* hadc,
 ...
 * HAL_ADC_STATE_REG_BUSY)) " " if (HAL_IS_BIT_SET(HAL_ADC_GetState(hadc1),
 * HAL_ADC_STATE_AWD1)    ) "
 * @param  hadc ADC handle
 * @retval HAL state
 */
