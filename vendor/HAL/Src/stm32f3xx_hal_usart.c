...
@endverbatim
  ...
  8-bit or 9-bit) or  by the M bit (8-bits or 9-bits), the possible USART
...
USART synchronous SPI data transfers.
    ...
    [..] The USART Synchronous SPI supports master mode only: it cannot receive
or send data related to an input clock (SCLK is always an output).
...
static HAL_StatusTypeDef USART_SetConfig(USART_HandleTypeDef *husart) {
  ...
  switch (clocksource) {
    ...
    case USART_CLOCKSOURCE_HSI:
      usartdiv =
          (uint32_t)(USART_DIV_SAMPLING8(HSI_VALUE, husart->Init.BaudRate));
      break;
    ...
    case USART_CLOCKSOURCE_LSE:
      usartdiv =
          (uint32_t)(USART_DIV_SAMPLING8(LSE_VALUE, husart->Init.BaudRate));
      break;
    default:
      ret = HAL_ERROR;
      break;
