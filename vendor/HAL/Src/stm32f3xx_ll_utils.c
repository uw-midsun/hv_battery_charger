...
#define UTILS_PLL_OUTPUT_MAX \
  72000000U /*!< Frequency max for PLL output, in Hz  */

/* Defines used for HSE range */
#define UTILS_HSE_FREQUENCY_MIN \
  4000000U /*!< Frequency min for HSE frequency, in Hz   */
#define UTILS_HSE_FREQUENCY_MAX \
  32000000U /*!< Frequency max for HSE frequency, in Hz   */

/* Defines used for FLASH latency according to SYSCLK Frequency */
#define UTILS_LATENCY1_FREQ \
  24000000U /*!< SYSCLK frequency to set FLASH latency 1 */
#define UTILS_LATENCY2_FREQ \
  48000000U /*!< SYSCLK frequency to set FLASH latency 2 */
/**
 * @}
 */
/* Private macros ------------------------------------------------------------*/
...
#define IS_LL_UTILS_PREDIV_VALUE(__VALUE__) \
  (((__VALUE__) == LL_RCC_PREDIV_DIV_1) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_2) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_3) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_4) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_5) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_6) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_7) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_8) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_9) ||  \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_10) || \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_11) || \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_12) || \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_13) || \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_14) || \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_15) || \
   ((__VALUE__) == LL_RCC_PREDIV_DIV_16))

#define IS_LL_UTILS_PLL_FREQUENCY(__VALUE__) \
  ((__VALUE__) <= UTILS_PLL_OUTPUT_MAX)
...
/**
 * @brief  This function configures system clock with HSI as clock source of the
 * PLL
 * @note   The application need to ensure that PLL is disabled.
 * @note   Function is based on the following formula:
 *         - PLL output frequency = ((HSI frequency / PREDIV) * PLLMUL)
 *         - PREDIV: Set to 2 for few devices
 *         - PLLMUL: The application software must set correctly the PLL
 * multiplication factor to not exceed 72MHz
 ...
 *         - PREDIV: Set to 2 for few devices
 *         - PLLMUL: The application software must set correctly the PLL
 * multiplication factor to not exceed @ref UTILS_PLL_OUTPUT_MAX
 * @note   FLASH latency can be modified through this function.
...
static uint32_t UTILS_GetPLLOutputFrequency(
  ...
  /* The application software must set correctly the PLL multiplication factor
     to not exceed @ref UTILS_PLL_OUTPUT_MAX */
#if defined(RCC_PLLSRC_PREDIV1_SUPPORT)
  pllfreq =
      __LL_RCC_CALC_PLLCLK_FREQ(PLL_InputFrequency, UTILS_PLLInitStruct->PLLMul,
                                UTILS_PLLInitStruct->PLLDiv);
#else
  pllfreq = __LL_RCC_CALC_PLLCLK_FREQ(
      PLL_InputFrequency / (UTILS_PLLInitStruct->Prediv + 1U),
      UTILS_PLLInitStruct->PLLMul);
#endif /*RCC_PLLSRC_PREDIV1_SUPPORT*/
...
static ErrorStatus UTILS_EnablePLLAndSwitchSystem(
  ...
  sysclk_frequency_current =
      (SystemCoreClock
       << AHBPrescTable[LL_RCC_GetAHBPrescaler() >> RCC_POSITION_HPRE]);
