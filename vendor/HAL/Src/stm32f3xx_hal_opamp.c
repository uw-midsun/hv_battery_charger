...
================================================================================
  ...
  [..] The device integrates up to 4 operational amplifiers OPAMP1, OPAMP2,
       ...
       (++) Runs automatically the calibration in 2 steps.
            (90U% of VDDA for NMOS transistors, 10U% of VDDA for PMOS
...
(multiplexer) mode can be set on top.
       (++) Timer-controlled Mux mode allows Automatic switching between
...
trimming values (typ. settings returned by HAL_OPAMP_SelfCalibrate function).
      ...
      (#) Lock the opamp in running mode using HAL_OPAMP_Lock() function. From
then The configuration can  be modified
      (++) After HW reset
...

{
  ...
  if (hopamp == NULL) {
    ...
    if ((hopamp->Init.TimerControlledMuxmode) ==
      ...
      if ((hopamp->Init.Mode) == OPAMP_STANDALONE_MODE) {
        assert_param(
            IS_OPAMP_SEC_INVERTINGINPUT(hopamp->Init.InvertingInputSecondary));
      }
    }
     ...
     */
    /*     Set  bits according to hopamp->hopamp->Init.TimerControlledMuxmode
     * value               */
    /*     Set  bits according to hopamp->hopamp->Init.InvertingInputSecondary
     * value             */
    ...
    if ((hopamp->Init.Mode == OPAMP_PGA_MODE) ||
      ...
      MODIFY_REG(hopamp->Instance->CSR, OPAMP_CSR_UPDATE_PARAMETERS_INIT_MASK,
                 hopamp->Init.Mode | hopamp->Init.NonInvertingInput |
...
HAL_StatusTypeDef HAL_OPAMP_SelfCalibrate(OPAMP_HandleTypeDef *hopamp) {
  ...
  if (hopamp == NULL) {
    ...
    if (hopamp->State == HAL_OPAMP_STATE_READY) {
      ...
      /* Still need to check if right calibration is current value or un step
       * below */
...
OPAMP_TrimmingValueTypeDef HAL_OPAMP_GetTrimOffset(OPAMP_HandleTypeDef *hopamp,
  ...
  OPAMP_TrimmingValueTypeDef oldtrimmingvaluep = 0U, oldtrimmingvaluen = 0U,
                             trimmingvalue = 0U;
