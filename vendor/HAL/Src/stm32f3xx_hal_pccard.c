/**
 ...
 to control PCCARD/compact flash memories. It uses the FMC layer functions to
 interface with PCCARD devices. This driver is used for:
 ...
 PCCARD/compact flash operations. If a PCCARD/Compact Flash device contains
 different operations and/or implementations, it should be implemented
 separately.
 ...
 ID and a pointer to the user callback function.
      ...
      Use function HAL_PCCARD_UnRegisterCallback() to reset a callback to the
 ...
 Callback ID.
      ...
      By default, after the HAL_PCCARD_Init and if the state is
 HAL_PCCARD_STATE_RESET all callbacks are reset to the corresponding legacy weak
 (surcharged) functions. Exception done for MspInit and MspDeInit callbacks that
 are respectively reset to the legacy weak (surcharged) functions in the
 HAL_PCCARD_Init and  HAL_PCCARD_DeInit only when these callbacks are null (not
 registered beforehand). If not, MspInit or MspDeInit are not null, the
 HAL_PCCARD_Init and HAL_PCCARD_DeInit keep and use the user MspInit/MspDeInit
 ...
 register the MspInit/MspDeInit user callbacks using HAL_PCCARD_RegisterCallback
 before calling HAL_PCCARD_DeInit or HAL_PCCARD_Init function.

      When The compilation define USE_HAL_PCCARD_REGISTER_CALLBACKS is set to 0
 ...
 *          @arg @ref HAL_PCCARD_MSP_DEINIT_CB_ID     PCCARD MspDeInit callback
 * ID
 *          @arg @ref HAL_PCCARD_IT_CB_ID             PCCARD IT callback ID
 * @param pCallback : pointer to the Callback function
 * @retval status
 */
HAL_StatusTypeDef HAL_PCCARD_RegisterCallback(
    PCCARD_HandleTypeDef *hpccard, HAL_PCCARD_CallbackIDTypeDef CallbackId,
 ...
 *         PCCARD Callback is redirected to the weak (surcharged) predefined
 * callback
 * @param hpccard : PCCARD handle
 ...
 * @retval status
 */
HAL_StatusTypeDef HAL_PCCARD_UnRegisterCallback(
    PCCARD_HandleTypeDef *hpccard, HAL_PCCARD_CallbackIDTypeDef CallbackId) {
  HAL_StatusTypeDef status = HAL_OK;
